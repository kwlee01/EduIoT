...
#   def __init__(self, ax,fn, xmax=10,ymax =200,xstart=0, ystart=-200,title='Angular Velocity',xlabel='Time [ms]',ylabel='Roll velocity'):
    def __init__(self,ax1,ax2,ax3,fn,xmax=10,xstart=0): 
...
#       self.y = [0] # y axis info
        ...
        ax1.grid(True)
        ax1.set_xlabel("Time")
        ax1.set_ylabel("Roll Velocity")
        #ax1.axis([0,100,-200.,200.])
        self.ax1.set_xlim((self.xstart,self.xmax))
        self.ax1.set_ylim((-200,200))
        line1=ax1.plot(self.x,self.y1,'-')

        ax2 = fig.add_subplot(312)
        ax2.grid(True)
        ax2.set_xlabel("Time")
        ax2.set_ylabel("Pitch Velocity")
#        ax2.axis([0,100,-200.,200.])
        self.ax2.set_xlim((self.xstart,self.xmax))
        self.ax2.set_ylim((-200,200))
        line2=ax2.plot(self.x,self.y2,'-')
        ...
        self.ax3.set_xlim((self.xstart,self.xmax))
        self.ax3.set_ylim((-200,200))
        line3=ax3.plot(self.x,self.y3,'-')
    ...
    def update(self, i):  # graph setting
#    def update(self, i):  # graph setting
        # time difference
...
#        self.x.append(tempo + self.x[-1]) # x insert
        ...
        if self.x[-1] >= self.xstart + self.xmax :
            ...
            self.ax1.set_xlim(self.xstart,self.xstart + self.xmax)
            self.ax1.figure.canvas.draw()
            self.ax2.set_xlim(self.xstart,self.xstart + self.xmax)
            self.ax2.figure.canvas.draw()
            self.ax3.set_xlim(self.xstart,self.xstart + self.xmax)
            fig.tight_layout()
...
# Creation of object
scope = Scope(ax1,ax2,ax3,insert)
